...
const char DEFAULT_WALLET_FILE_NAME[] = "scashwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.scash.org:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("scashwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("scashwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("scashwallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("scashWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.scash.scashwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=scash Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("scashWallet", appPath);
    } else {
      autorunSettings.remove("scashWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\scash", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:scash");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\scash\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\scash\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("scashwallet.cfg"));
