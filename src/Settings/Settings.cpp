...
const char DEFAULT_WALLET_FILE_NAME[] = "securecashwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.getsecurecash.org:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("securecashwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("securecashwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("securecashwallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("securecashWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.securecash.securecashwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=securecash Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("securecashWallet", appPath);
    } else {
      autorunSettings.remove("securecashWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\securecash", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:securecash");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\securecash\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\securecash\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("securecashwallet.cfg"));
