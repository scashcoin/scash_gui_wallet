...
void WalletStatusBar::synchronizationProgressUpdated(quint32 _current, quint32 _total) {
  ...
  m_walletIsSynchronized = false;
  m_syncStatusLabel->setText(tr("  Synchronizing with Secure Cash network: %1/%2").arg(_current).arg(_total).arg(blockchainAge));
...
void WalletStatusBar::updateStatusDescription() {
  ...
  QString statusText = tr("Synchronized with Secure Cash network       Blockchain height: %1       Found (UTC): %2%3");
