...
bool WalletApplication::init() {
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Secure Cash wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "SecureCash wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have Secure Cash with non-default RPC port, you should terminate it and relaunch Secure Cash GUI Wallet\n"
      "or\n"
