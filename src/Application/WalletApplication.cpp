...
bool WalletApplication::init() {
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] eGameCash wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "eGameCash wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have eGameCashd with non-default RPC port, you should terminate it and relaunch eGameCash Lite\n"
      "or\n"
